...
import { Bell, Globe, AlertTriangle, CheckCircle, Activity, X } from 'lucide-react';
...
export default function TopBar({ lang, onLangToggle, pageTitle }: Props) {
  ...
  return (
    <header className="h-12 bg-navy-900 border-b border-navy-700/60 flex items-center justify-between px-5 shrink-0 gap-4">
      {/* Left — page title */}
      ...
      {/* Center — disclaimer */}
      <div className="hidden md:flex items-center gap-2 px-3 py-1 rounded-full bg-amber-950/30 border border-amber-800/25">
        <span className="text-[10px] text-amber-400/70 font-medium">⚠ Synthetic / De-identified Data — Not Live CCTNS/ICJS</span>
      </div>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className={`btn-ghost relative p-2 ${showNotifs ? 'bg-navy-800 text-slate-200' : ''}`}
            title="Notifications"
          >
            <Bell className="w-4 h-4" />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-amber-500 text-navy-950 text-[9px] font-bold flex items-center justify-center">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {/* Dropdown */}
          {showNotifs && (
            <div className="absolute right-0 top-full mt-1.5 w-96 bg-navy-900 border border-navy-700 rounded-xl shadow-2xl z-50 overflow-hidden">
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-navy-700">
                <div>
                  <span className="text-sm font-semibold text-slate-100">Notifications</span>
                  {unreadCount > 0 && (
                    <span className="ml-2 text-[10px] bg-amber-500/20 text-amber-400 border border-amber-500/30 px-1.5 py-0.5 rounded-full font-semibold">
                      {unreadCount} new
                    </span>
                  )}
                </div>
                {unreadCount > 0 && (
                  <button onClick={markAllRead} className="text-[10px] text-amber-400 hover:text-amber-300 transition-colors">
                    Mark all read
                  </button>
                )}
              </div>
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map(n => (
                    <div
                      key={n.id}
                      className={`flex gap-3 px-4 py-3 border-b border-navy-800/60 hover:bg-navy-800/40 transition-colors ${!n.read ? 'bg-navy-800/20' : ''}`}
                    >
                      <div className="mt-0.5 shrink-0">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                          n.type === 'alert' ? 'bg-orange-900/40' :
                          n.type === 'update' ? 'bg-emerald-900/40' :
                          'bg-blue-900/40'
                        }`}>
              ...
              <div className="px-4 py-2.5 border-t border-navy-700 text-center">
                <span className="text-[10px] text-slate-600">System alerts only — not live intelligence feeds</span>
              </div>
            </div>
          )}
        </div>
        ...
        {/* User badge */}
        <div className="flex items-center gap-2 pl-3 border-l border-navy-700/60">
          <div className="w-7 h-7 rounded-full bg-navy-800 border border-navy-600 overflow-hidden flex items-center justify-center">
            <img
